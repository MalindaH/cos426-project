...
//   vec2 coords = (gl_PointCoord - 0.5) * mat2(vAngle.x, vAngle.y, -vAngle.y, vAngle.x) + 0.5;
  gl_FragColor = vColour; // * texture2D(diffuseTexture, coords)
}`;
...
class ParticleSystem extends Group {
    constructor(parent, camera) {
        ...
        const uniforms = {
            // diffuseTexture: {
            //     value: new THREE.TextureLoader().load('./src/textures/water.png'),
            // },
            pointMultiplier: {
                value:
                    window.innerHeight /
    ...
    _AddParticles(timeElapsed, y) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            this._particles.push({
                ...
                velocity: new THREE.Vector3(Math.random()*0.2-0.1, 8, Math.random()*0.2-0.1),
            });
        }
    }
    ...
    Step(timeElapsed, y) {
        this._AddParticles(timeElapsed, y);
